...
if __name__ == "__main__":
    ...
    logger = Logger()
    criterion_loader = CriterionLoader("resources/criteria.json")
    dropbox_client = DropboxClient(logger=logger)
    ...
    for product in product_names:
        ...
        for match in matches:
            if not re_pattern.search(os.path.splitext(metadata.name)[1]):
                continue 
            ...
            file_record = FileRecord(
                ...
                extension=os.path.splitext(metadata.name)[1],
                metadata={
                    "id": metadata.id,
                    "client_modified": metadata.client_modified,
                    "rev": metadata.rev,
                    "resolution": metadata.media_info.get("metadata", {}).get("dimensions")[0]*metadata.media_info.get("metadata", {}).get("dimensions")[1] if metadata.media_info else None
                }
            )
